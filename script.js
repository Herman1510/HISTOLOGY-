...
document.addEventListener("DOMContentLoaded", function() {
    document.getElementById("quizForm").addEventListener("submit", function(event) {
        ...
        let percentage = ((score / Object.keys(correctAnswers).length) * 100).toFixed(2);
        let grade = percentage >= 80 ? "A" : percentage >= 70 ? "B" : percentage >= 60 ? "C" : percentage >= 50 ? "D" : "F";
        let comments = grade === "A" ? "Excellent performance!" :
            grade === "B" ? "Good job!" :
            grade === "C" ? "Fair effort, keep improving!" :
            grade === "D" ? "You passed, but aim higher!" :
        ...
        let resultHTML = `
            ...
            <p><strong>Score:</strong> ${score} / 50 (${percentage}%)</p>
